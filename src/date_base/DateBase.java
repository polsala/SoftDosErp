...
public class DateBase {
    ...
    /* TODO
    */
    public Vector<Object> search_in_table_by_value(String table_name, String atribute_name, Object atribute_value){
       Map<Object,Object> table = search_table(table_name);
       if(table != null){
            Vector<Object> value = new Vector<>();
            try{
                Class c_db = Class.forName("date_base."+ table_name);
                for(Object ob : table.values()){
                    Object casted_o = c_db.cast(ob);
                    Field f = casted_o.getClass().getField(atribute_name);
                    //f.setAccessible(true);
                    Object it_tab = f.get(casted_o);
                    if(it_tab.equals(atribute_value)){
                        value.add(casted_o);
                    }
                }
                if (!value.isEmpty()){
                    ...
                    System.err.println("No s'ha trobat cap coincidencia");
                }
            }catch (Exception e){
                System.err.println("No s'ha pogut buscar");
            }
       }else{
